...
namespace ct {
	...
	public:
		...
		CT_CONSTEXPR tmat3();
		CT_CONSTEXPR tmat3(const tmat3<T> & other);

		// Explict constructors

		CT_CONSTEXPR explicit tmat3(T identity);
		...
		CT_CONSTEXPR const column_type & col(const u32 & index) const;
		CT_CONSTEXPR column_type & col(const u32 & index);

		CT_CONSTEXPR column_type get_column(u32 index) const;
		CT_CONSTEXPR row_type get_row(u32 index) const;

		CT_CONSTEXPR void set(T m00, T m10, T m20, T m01, T m11, T m21, T m02, T m12, T m22);
		CT_CONSTEXPR void set_column(u32 index, const column_type & column);
		CT_CONSTEXPR void set_row(u32 index, const row_type & row);

		CT_CONSTEXPR T determinant() const;
		CT_CONSTEXPR tmat3<T> inverse() const;
		CT_CONSTEXPR void transpose();

		// Assign operators

		template <typename U>
		CT_CONSTEXPR tmat3<T> & operator=(const tmat3<U> & other);
		...
		template <typename U>
		CT_CONSTEXPR tmat3<T> & operator+=(U scalar);
		...
		////////////////////////////////////////////////////////////
		CT_CONSTEXPR T & operator()(const u32 & x, const u32 & y);
		...
		/// @param y column index
		////////////////////////////////////////////////////////////
		CT_CONSTEXPR const T & operator()(const u32 & x, const u32 & y) const;
	...
	template <typename T>
	CT_CONSTEXPR tmat3<T> operator+(const tmat3<T> & mat);
	...
	template <typename T>
	CT_CONSTEXPR tmat3<T> operator+(const tmat3<T> & other, T scalar);

	template <typename T>
	CT_CONSTEXPR tmat3<T> operator+(T scalar, const tmat3<T> & other);

	template <typename T>
	CT_CONSTEXPR tmat3<T> operator+(const tmat3<T> & left, const tmat3<T> & right);
	...
	template <typename T>
	CT_CONSTEXPR typename tmat3<T>::column_type operator*(const tmat3<T> & mat, typename tmat3<T>::row_type const & row_vec);

	template <typename T>
	CT_CONSTEXPR typename tmat3<T>::row_type operator*(typename tmat3<T>::column_type const & col_vec, const tmat3<T> & mat);
